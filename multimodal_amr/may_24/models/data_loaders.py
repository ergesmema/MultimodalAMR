...
class DrugResistanceDataset(Dataset):
    def __init__(self, long_table_df, spectra_matrix, drugs_df, samples_list):
        ...
        self.drug2idx = {d: i for i, d in self.idx2drug.items()}
         ...
         # Define mappings for dataset and year
        self.dataset2idx = {'A': 0, 'B': 1, 'C': 2, 'D': 3}
        self.year2idx = {2015: 0, 2016: 1, 2017: 2, 2018: 3}

    def __len__(self):
        return len(self.long_table)

    def __getitem__(self, idx):
        species, sample_id, drug_name, response, dataset, year = self.long_table.iloc[idx]
        
        # Convert drug names and sample IDs to tensors using mappings
        ...
        return species_idx, spectrum, fprint_tensor, response, dataset_idx, year_idx
