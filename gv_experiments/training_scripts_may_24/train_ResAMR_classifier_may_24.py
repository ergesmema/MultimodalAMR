...
import itertools
from multimodal_amr.may_24.data_split.data_utils import DataSplitter
...
#Change the data path to the folder where you hold the datasets
DATA_PATH = "/Users/em/Desktop/Uni-Spring24/XAIML-gitlab/b4-interpretable-antimicrobial-recommendation/MultimodalAMR/data"
    ...
    if not os.path.exists(target_dir):
        ...
        for index, row in filtered_df.iterrows():
            ...
            for year in years:
                ...
                source_path = os.path.join(DATA_PATH, source_dir, file_name)
                # print(source_path)
        ...
        np.save(os.path.join(DATA_PATH, "spectra_binned_6000_all.npy"), spectra_matrix)
        # Convert the list to a DataFrame
        df = pd.DataFrame(sample_ids, columns=['sample_id', 'year'])
        # Save the DataFrame to a CSV file
        df.to_csv(os.path.join(DATA_PATH, "existing_sample_ids.csv"), index=False)
...
def main(args): 
    ...
    existing_sample_ids = pd.read_csv(os.path.join(DATA_PATH, "existing_sample_ids.csv"))
